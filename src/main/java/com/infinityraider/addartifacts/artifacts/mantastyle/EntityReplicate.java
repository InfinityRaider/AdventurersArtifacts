...
import com.infinityraider.addartifacts.reference.Names;
...
public class EntityReplicate extends EntityLiving implements IEntityAdditionalSpawnData {
    ...
    public static List<EntityReplicate> getReplicates(EntityPlayer player) {
        if(!player.getEntityWorld().isRemote && player instanceof EntityPlayerMP) {
            if(!replicaMap.containsKey(player.getUniqueID())) {
                replicaMap.put(player.getUniqueID(), new ArrayList<>());
            }
            return ImmutableList.copyOf(replicaMap.get(player.getUniqueID()));
        }
    ...
    public AbstractClientPlayer getClientPlayer() {
        ...
        return (AbstractClientPlayer) AddArtifacts.proxy.getClientPlayer();
    }
