...
public class RenderHexedHandler {
    ...
    private ResourceLocation pigTexture = new ResourceLocation("textures/entity/pig/pig.png");
    private ModelPig pigModel = new ModelPig();

    private RenderHexedHandler() {
        this.hexedEntities = Sets.newHashSet();
    }

    protected void addHexedEntity(EntityLivingBase entity) {
        this.hexedEntities.add(entity.getEntityId());
    }

    protected void removeHexedEntity(int id) {
    ...
    public void onRenderLiving(RenderLivingEvent.Pre event) {
        ...
        if(hexedEntities.contains(entity.getEntityId())) {
            ...
            this.renderPig(event.getRenderer(), entity, event.getX(), event.getY(), event.getZ(), Minecraft.getMinecraft().getRenderPartialTicks());
        }
    }

    //Mostly copied from vanilla at this point
    @SuppressWarnings("unchecked")
    protected void renderPig(RenderLivingBase renderer, EntityLivingBase entity, double x, double y, double z, float partialTicks) {
        GlStateManager.pushMatrix();
        GlStateManager.disableCull();
        this.pigModel.swingProgress = entity.getSwingProgress(partialTicks);
        boolean shouldSit = entity.isRiding() && (entity.getRidingEntity() != null && entity.getRidingEntity().shouldRiderSit());
        this.pigModel.isRiding = shouldSit;
        this.pigModel.isChild = entity.isChild();
        try {
            ...
            this.pigModel.setLivingAnimations(entity, f6, f5, partialTicks);
            this.pigModel.setRotationAngles(f6, f5, timeExisted, yawHead, rotationPitch, f4, entity);
    ...
    protected void renderModel(RenderLivingBase renderer, EntityLivingBase entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor) {
        ...
        boolean renderGhostly = !doRender && !entity.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer);
        if (doRender || renderGhostly) {
            renderer.getRenderManager().renderEngine.bindTexture(this.pigTexture);
            if (renderGhostly) {
                GlStateManager.enableBlendProfile(GlStateManager.Profile.TRANSPARENT_MODEL);
            }
            this.pigModel.render(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor);
            if (renderGhostly) {
